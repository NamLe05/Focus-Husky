import {app, BrowserWindow, ipcMain, dialog, IpcMainEvent, WebContents, HandlerDetails} from 'electron';
import TypedDatastore from './services/db';
import {TaskState} from './tasks/model';
import {PetState} from './pet/model';
import {GlobalPetController} from './pet/globalController';
...
const createWindow = (): void => {
  ...
  mainWindow.webContents.session.webRequest.onHeadersReceived(
    (details: HandlerDetails, callback: (response: {responseHeaders: Record<string, string>}) => void) => {
      callback({
        responseHeaders: {
...
const globalController = GlobalPetController.getInstance();
...
// Set up IPC handlers for pet actions
ipcMain.on('feed-pet', (event: IpcMainEvent, petId: string) => {
  globalController.handleFeedPet(petId);
});

ipcMain.on('play-with-pet', (event: IpcMainEvent, petId: string) => {
  globalController.handlePlayWithPet(petId);
});

ipcMain.on('groom-pet', (event: IpcMainEvent, petId: string) => {
  globalController.handleGroomPet(petId);
});

ipcMain.on('move-pet', (event: IpcMainEvent, petId: string, x: number, y: number) => {
  globalController.handleMovePet(petId, x, y);
});

ipcMain.on('pomodoro-completed', (event: IpcMainEvent, petId: string) => {
  globalController.handlePomodoroCompleted(petId);
});

ipcMain.on('task-completed', (event: IpcMainEvent, petId: string) => {
  globalController.handleTaskCompleted(petId);
});

ipcMain.on('load-pets', () => {
  globalController.loadPetsFromDatabase();
});

ipcMain.handle('get-all-pets', () => {
  return globalController.getAllPets();
});
